...
function cleanXmlString(xmlString: string): string {
  // Remove any characters before the XML declaration or root tag
  ...
  for (const url of urlsToTry) {
    try {
      ...
      try {
        ...
        console.warn(`Direct parse failed for ${url}, trying with fetch + clean:`, parseError);
        // If direct parse fails, try fetching the content first, clean it, then parse
      ...
      console.warn(`Failed to fetch from ${url} (attempt ${retryCount + 1}):`, error);
      
      // If we have retries left, wait a bit and try again
      if (retryCount < 2) {
        await new Promise(resolve => setTimeout(resolve, 1000 * (retryCount + 1)));
        return fetchRssFeedServer(scope, source, retryCount + 1);
      }
